...
@app.route('/unknownYelpIDs')
def unknownYelpIDs():
	return send_from_directory('', 'unknownYelpIDs.csv')

@app.route('/unknownProcedures')
def unknownProcedures():
	return send_from_directory('', 'unknownProcedures.csv')

@app.route('/secretsecret')
def sendDataToVetCompare():
	# Setup csv files
	...
	with open('unknownProcedures.csv', 'wb') as csvfile:
		writer = csv.writer(csvfile)
		writer.writerow(['procedure_name'])
		...
		if entry.procedure:
			...
			if not price:
				with open('unknownProcedures.csv', 'ab') as csvfile:
					writer = csv.writer(csvfile)
					writer.writerow([entry.procedure])
				continue
			## Calculate percentiles ##
			# National
